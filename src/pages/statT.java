...
public class statT extends JFrame {
    ...
    private int[][] getTempDataFromDatabase() {
        ...
        // SQL query to fetch the temperature data for each week and day
        String query = "SELECT week, day, Temp FROM status WHERE ID_patient = '" + GlobalData.id_pa + "';";
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASSWORD);
            ...
            while (rs.next()) {
                ...
                int Temp = rs.getInt("Temp");  // Fetching the Temp column

                // Store the Temp value in the array (default 0 if not found)
                if (week >= 1 && week <= 7 && day >= 1 && day <= 7) {
                    Data[week - 1][day - 1] = Temp;  // Store the Temp value
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
