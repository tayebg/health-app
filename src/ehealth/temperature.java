...
public class temperature extends JFrame {
    ...
    public temperature() {
        setTitle("Temperature - E-Health App");
        ...
        // Title Label
        JLabel lblTitle = new JLabel("Weekly Temperature Overview (°C)");
    ...
    private int[][] loadTemperatureData() {
        int[][] data = new int[4][7];
        for (int w = 0; w < 4; w++) {
            for (int d = 0; d < 7; d++) {
                data[w][d] = (globaldata.Temp != null && w < globaldata.Temp.length && d < globaldata.Temp[w].length)
                    ? globaldata.Temp[w][d] : 37;
            }
        }
    ...
    private JPanel createGraphPanel() {
        ...
        for (int week = 0; week < 4; week++) {
            ...
            for (int day = 0; day < 7; day++) {
                int tempVal = data[week][day];
                int barHeight = Math.max(10, Math.min(260, (tempVal - 20) * 10));

                JPanel bar = new JPanel();
                bar.setBackground(new Color(255, 75, 120));
                bar.setBounds(xOffset + day * (barWidth + spacing), graphBottom - barHeight, barWidth, barHeight);
                bar.setToolTipText("Week " + (week + 1) + " Day " + (day + 1) + ": " + tempVal + "°C");
                panel.add(bar);

                JLabel valLabel = new JLabel(String.valueOf(tempVal), SwingConstants.CENTER);
