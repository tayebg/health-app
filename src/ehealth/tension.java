...
public class tension extends JFrame {
    ...
    public tension() {
        ...
        // Title Label
        JLabel lblTitle = new JLabel("Weekly Tension Overview (mmHg)");
    ...
    private int[][] loadTensionData() {
        int[][] data = new int[4][7];
        for (int w = 0; w < 4; w++) {
            for (int d = 0; d < 7; d++) {
                data[w][d] = (globaldata.ten != null && w < globaldata.ten.length && d < globaldata.ten[w].length)
                    ? globaldata.ten[w][d] : 120;
            }
        }
    ...
    private JPanel createGraphPanel() {
        ...
        for (int week = 0; week < 4; week++) {
            ...
            for (int day = 0; day < 7; day++) {
                int tenVal = data[week][day];
                // Scale bar height for 10-180 mmHg
                int barHeight = Math.max(10, Math.min(260, (int)(tenVal * 1.5)));
                if (tenVal < 20) barHeight = Math.max(10, tenVal * 14); // If user enters e.g. 12 instead of 120

                JPanel bar = new JPanel();
                bar.setBackground(new Color(160, 70, 240));
                bar.setBounds(xOffset + day * (barWidth + spacing), graphBottom - barHeight, barWidth, barHeight);
                bar.setToolTipText("Week " + (week + 1) + " Day " + (day + 1) + ": " + tenVal + " mmHg");
                panel.add(bar);

                JLabel valLabel = new JLabel(String.valueOf(tenVal), SwingConstants.CENTER);
